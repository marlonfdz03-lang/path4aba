...
test('MARLON REQUIRED: DISJOINT registers — no response OR prompt category shared between ADJACENT tiers', () => {
  const shares = (a, b) => a.some((x) => b.includes(x));
  // The overlap was the real cause of typical/below reading identically.
  assert.ok(!shares(TIER_RESPONSES.FAVORABLE, TIER_RESPONSES.PARTIAL), 'FAVORABLE vs PARTIAL responses are disjoint');
  assert.ok(!shares(TIER_RESPONSES.PARTIAL, TIER_RESPONSES.DIFFICULT), 'PARTIAL vs DIFFICULT responses are disjoint');
  assert.ok(!shares(TIER_PROMPTS.FAVORABLE, TIER_PROMPTS.PARTIAL), 'FAVORABLE vs PARTIAL prompts are disjoint');
  assert.ok(!shares(TIER_PROMPTS.PARTIAL, TIER_PROMPTS.DIFFICULT), 'PARTIAL vs DIFFICULT prompts are disjoint');
  // No evaluative language leaked into any register.
  const flat = [...Object.values(TIER_RESPONSES).flat(), ...Object.values(TIER_PROMPTS).flat()].join(' ');
  assert.ok(!/success|improv|excellent/i.test(flat), 'no evaluative words in the registers');
  // DIFFICULT does not assert a behavioral outcome (no "failed/refused/incomplete/disengaged").
  assert.ok(!/fail|refus|incomplete|disengage/i.test(TIER_RESPONSES.DIFFICULT.join(' ')), 'DIFFICULT states support, not outcome');
});
