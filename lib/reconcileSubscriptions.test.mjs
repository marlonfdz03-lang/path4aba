...
import { reconcilePrimary, reconcileBcba, shouldAbort, ABORT_MIN_ROWS, parseDryRun } from './reconcileSubscriptions.ts';
...
// Regression for the phantom trial_ends_at writes: same UTC day, different time-of-day is NOT drift.
test('reconcilePrimary: same calendar day at a different time is NOT a drift (no phantom write)', () => {
  // fixture item period = 1790273860 = 2026-09-24T00:37:40Z; row is the same day at a different time.
  const row = { plan: 'rbt_1', status: 'active', current_period_ends_at: new Date('2026-09-24T18:00:00Z'), trial_ends_at: null };
  const { data, drifts } = reconcilePrimary(row, sub, PRICE_TO_PLAN);
  assert.equal(drifts.length, 0);
  assert.deepEqual(data, {});
});
...
// ── parseDryRun: the fail-safe flag. Absent = write; recognized truthy = dryrun; anything else = abort. ──
test('parseDryRun: absent param means WRITE (the cron mode — no query string)', () => {
  assert.equal(parseDryRun(false, null), 'write');
});

test('parseDryRun: recognized truthy forms mean DRYRUN, incl. bare presence and mixed case/space', () => {
  for (const v of ['1', 'true', 'yes', '', 'TRUE', ' Yes ']) {
    assert.equal(parseDryRun(true, v), 'dryrun', `expected dryrun for ${JSON.stringify(v)}`);
  }
  assert.equal(parseDryRun(true, ''), 'dryrun'); // ?dryRun with no value
});

test('parseDryRun: present-but-unrecognized ABORTS — never writes (the ?dryRun=1 lesson)', () => {
  for (const v of ['0', 'false', 'no', 'maybe', 'write', '2']) {
    assert.equal(parseDryRun(true, v), 'abort', `expected abort for ${JSON.stringify(v)}`);
  }
});
