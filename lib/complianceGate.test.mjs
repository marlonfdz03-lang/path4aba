...
// When segmentation is UNSOUND, generateSmartNote marks coverage `suppressed` (raw reading retained, not
// fabricated) and drops validity. SUPPRESSED is a third state, distinct from CLEAN and DEFECTIVE. These tests
// prove: (a) a suppressed state is distinguishable from a clean one, (b) neither a suppressed coverage nor a
// suppressed+raw-missing coverage produces a repair, and (c) the whole-note checks still repair.
test('segmentation-unsound suppression is distinct from clean, never repairs, and does not disable whole-note checks', () => {
  // A suppressed coverage carries the raw (untrusted) reading — here a NON-empty missing list — plus the reason.
  const suppressed = clean({
    coverage: { segmentable: true, missing: [{ name: 'Elopement' }, { name: 'Off-task Behavior' }], suppressed: 'degenerate' },
    functionViolations: [],
  });
  // (a) distinguishable from clean via summarize
  const sumClean = summarizeSurvivingViolations(clean());
  const sumSupp = summarizeSurvivingViolations(suppressed);
  assert.equal(sumClean.violations.coverageSuppressed, null, 'clean carries no suppression');
  assert.equal(sumSupp.violations.coverageSuppressed, 'degenerate', 'suppressed carries the reason — distinguishable from clean');
  assert.deepEqual(sumSupp.violations.coverageMissing, [], 'suppressed coverage reports NO missing (raw reading not judged)');
  assert.equal(sumSupp.violations.unsegmentable, false, 'suppression is not conflated with unsegmentable-not-clean');
  // (b) neither the suppressed state NOR its raw missing triggers a repair
  assert.equal(buildComplianceRegenInstruction(suppressed), null, 'suppressed coverage never repairs, even with raw missing present');
  // (c) same suppression + a real intervention defect → repair fires (whole-note, segmentation-independent)
  const withIntervention = clean({
    coverage: { segmentable: true, missing: [{ name: 'Elopement' }], suppressed: 'degenerate' },
    functionViolations: [],
    intervention: { prohibited: ['RIRD'], unapproved: [], skillAsReduction: [] },
  });
  assert.ok(buildComplianceRegenInstruction(withIntervention)?.includes('APPROVED INTERVENTIONS'), 'intervention defect still triggers a repair while coverage/validity are suppressed');
  // ...and a teaching-method defect → repair fires too
  const withMethod = clean({ coverage: { segmentable: true, missing: [], suppressed: 'unsegmentable' }, functionViolations: [], methodViolations: ['DTT'] });
  assert.ok(buildComplianceRegenInstruction(withMethod)?.includes('TEACHING METHODS'), 'teaching-method defect still triggers a repair');
});
