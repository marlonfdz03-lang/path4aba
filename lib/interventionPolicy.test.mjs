...
// ── REPORTED-CONTEXT EXEMPTION ────────────────────────────────────────────────────────────────
// The note form's environmental-change field is session CONTEXT ("a family member was visiting"),
// not an intervention the RBT performed. The prompt documents it as context; these lock the gate
// backstop for a model slip, and lock that the exemption is SCOPED — it must not become a hole.

const ENV_MOD_NOTE =
  'When the transition directive was presented, the client dropped to the floor, consistent with ' +
  'the documented escape function; the RBT implemented Environmental Modification by reducing ' +
  'visual distractions and providing a visual schedule.';

test('reported environmental change: env-modification phrasing does NOT hard-stop the note', () => {
  const gated = findInterventionViolations(ENV_MOD_NOTE, ['DRA', 'FCT'], []);
  assert.ok(gated.unapproved.includes('Environmental Modification'),
    'without reported context it is still gated exactly as before');

  const exempt = findInterventionViolations(ENV_MOD_NOTE, ['DRA', 'FCT'], [], { reportedEnvironmentalChange: true });
  assert.deepEqual(exempt.unapproved, [], 'reported context must never hard-stop the note');
  assert.ok(exempt.detected.includes('Environmental Modification'),
    'detection is unchanged — only the unapproved verdict is exempt');
});

test('the exemption does NOT weaken the real gate', () => {
  // Prohibited procedures are never exempt, with or without reported context.
  const rird = findInterventionViolations(
    ENV_MOD_NOTE + ' The RBT then implemented RIRD to interrupt the behavior.',
    ['DRA'], [], { reportedEnvironmentalChange: true },
  );
  assert.ok(rird.prohibited.includes('RIRD'), 'RIRD is still hard-prohibited under the exemption');

  // A genuinely out-of-plan intervention is still caught in the same note.
  const other = findInterventionViolations(
    ENV_MOD_NOTE + ' The RBT also implemented DRO by reinforcing the omission of the behavior.',
    ['DRA'], [], { reportedEnvironmentalChange: true },
  );
  assert.ok(other.unapproved.includes('DRO'), 'an unrelated unapproved intervention is still flagged');
  assert.ok(!other.unapproved.includes('Environmental Modification'),
    'only the reported-context intervention is exempt');
});

test('environmental manipulation / arrangement map onto the same approved key', () => {
  // The assessment extractor preserves the plan's wording ("Environmental Manipulation"), so an
  // approving plan must match a note that says "Environmental Modification" and vice versa.
  for (const approvedName of ['Environmental Manipulation', 'Environmental Modification', 'Environmental Arrangement']) {
    const v = findInterventionViolations(ENV_MOD_NOTE, [approvedName], []);
    assert.deepEqual(v.unapproved, [], `a plan approving "${approvedName}" must not flag the note`);
  }
  assert.ok(detectInterventions('the RBT implemented environmental manipulation by dimming the lights')
    .includes('Environmental Modification'), 'manipulation phrasing is detected');
  assert.ok(detectInterventions('the RBT implemented antecedent manipulation before the demand')
    .includes('Antecedent Modification'), 'antecedent manipulation is detected');
});

test('widened env/antecedent patterns do not match innocent prose', () => {
  const innocent = [
    'the client responded well to changes in the environment during the session',
    'an environmental change was reported by the caregiver at the start of the session',
    'the session environment was noisy while a family member was visiting',
    'the RBT noted the antecedent before each occurrence of the behavior',
  ];
  for (const text of innocent) {
    const detected = detectInterventions(text);
    assert.deepEqual(detected, [], `"${text}" must detect no intervention, got [${detected.join(', ')}]`);
...
// ── BEHAVIOR MOMENTUM ALIASES ────────────────────────────────────────────────────────────────
// One procedure, many plan spellings. A plan that approves it under ANY of its names must map onto
// the canonical key, or the gate flags a note the BCBA actually authorized. Widening detection is
// only safe because the prompt no longer teaches this intervention to clients who lack it.

test('every real plan spelling of Behavior Momentum maps onto one key', () => {
  const spellings = [
    'Behavior Momentum', 'Behavioral Momentum', 'Behavioural Momentum',
    'High-Probability Request Sequence', 'High Probability Request Sequence',
    'High-P Request Sequence', 'High-Probability Requests', 'High-P Sequence',
    'High-Probability Instructional Sequence', 'High-Probability Command Sequence',
    'High-Probability Demands', 'High-P Instruction',
  ];
  const note = 'the RBT implemented Behavior Momentum by presenting simple motor requests before the transition directive';
  for (const s of spellings) {
    const v = findInterventionViolations(note, [s, 'DRA'], []);
    assert.deepEqual(v.unapproved, [], `a plan approving "${s}" must not flag a Behavior Momentum note`);
  }
});

test('momentum prose in a note is detected under its aliases', () => {
  const prose = [
    'the RBT implemented behavioural momentum before the demand',
    'the RBT presented a high-probability instructional sequence prior to the directive',
    'the RBT delivered high-probability demands before re-presenting the task',
  ];
  for (const p of prose) {
    assert.ok(detectInterventions(p).includes('Behavior Momentum'), `"${p}" must be detected`);
  }
});

test('the widened momentum pattern does NOT match innocent prose', () => {
  // AGENTS.md: match the asserted PROCEDURE, never a bare noun or an ordinary probability phrase.
  const innocent = [
    'there is a high probability of escape-maintained behavior during demands',
    'the behavior occurred with high probability during transitions',
    'the client responded with high probability to familiar instructions',
    'momentum in the session improved after the break',
    'the RBT presented a demand and the client complied',
  ];
  for (const text of innocent) {
    const detected = detectInterventions(text);
    assert.deepEqual(detected, [], `"${text}" must detect no intervention, got [${detected.join(', ')}]`);
