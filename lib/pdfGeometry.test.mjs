...
import { readTargetList, readReplacementRoster, readPreferenceTable, readReduceTargets, readObjectivesStatus, redactText } from './pdfGeometry.ts';
import { buildFixedAssignmentsBlock } from './preselect.ts';
...
// ── redactText: the DEFAULT (fastMasTranscribe / assessment STRIP-BEFORE-LLM) is unchanged; a new namesOnly
// mode (note-generation prompt path) redacts identifiers ONLY, leaving pronouns and caregiver-relation words.
const hasWord = (t, w) => new RegExp(`\\b${w}\\b`, 'i').test(t);

test('redactText DEFAULT (unchanged): redacts name AND pronouns AND caregiver words (fastMAS behavior)', () => {
  const out = redactText('Any instance when Damian hits his mother during play', ['Damian']);
  assert.ok(!hasWord(out, 'Damian'), 'name gone');
  assert.ok(!hasWord(out, 'his'), 'pronoun gone (default)');
  assert.ok(!hasWord(out, 'mother'), 'caregiver word gone (default)');
});

test('redactText namesOnly: redacts the name but KEEPS pronouns and caregiver words (prompt path)', () => {
  const out = redactText('Any instance when Damian hits his mother during play', ['Damian'], { namesOnly: true });
  assert.ok(!hasWord(out, 'Damian'), 'name gone');
  assert.ok(hasWord(out, 'his'), 'pronoun preserved');
  assert.ok(hasWord(out, 'mother'), 'caregiver word preserved');
  assert.match(out, /^Any instance when the client hits his mother during play$/);
});

test('redactText namesOnly with an EMPTY names list leaves a name intact — this is WHY the prompt path fails loud on empty names', () => {
  const raw = 'Any instance when Damian climbs furniture';
  assert.equal(redactText(raw, [], { namesOnly: true }), raw, 'no names → nothing removed; caller must not proceed');
});

// ── The prompt scrub covers BOTH occurrences: the topography string that JSON.stringify feeds the USER prompt
// (sessionContext.behaviorsObserved) and the SYSTEM block (buildFixedAssignmentsBlock echoes a.topography).
// ── PHI FIREWALL (write side): mapToLegacyFormat (create/extract) and the behavior-topography PATCH route
// (human edit) both scrub a topography with EXACTLY this call before storing it. Locks the contract: the client
// name → "the client", pronouns + clinical vocabulary survive, and an EMPTY names list (first upload, no name
// available yet) is a documented fail-OPEN at the write (the prompt firewall is the guarantee at the model).
test('write-path (create / human-edit) contract: name scrubbed, pronoun + vocab survive', () => {
  const typed = 'Any instance when Damian climbs furniture and hits his mother';
  const out = redactText(typed, ['Damian'], { namesOnly: true });
  assert.ok(!hasWord(out, 'Damian'), 'name gone');
  assert.ok(hasWord(out, 'his') && hasWord(out, 'mother') && hasWord(out, 'climbs'), 'pronoun + caregiver-word + vocab survive');
});

test('write-path first upload (no name available) → fail-OPEN: name left intact by the write scrub', () => {
  const typed = 'Any instance when Damian climbs furniture';
  assert.equal(redactText(typed, [], { namesOnly: true }), typed, 'empty knownNames → write not scrubbed (documented)');
});

test('scrubbed topography is name-free in BOTH the user JSON and the fixed-assignments block', () => {
  const NAME = 'Damian';
  const rawTopo = `Any instance when ${NAME} climbs furniture and high places`;
  const scrubbed = redactText(rawTopo, [NAME], { namesOnly: true });
  // USER-JSON side: the value sessionContext.behaviorsObserved[i].topography would carry.
  assert.ok(!hasWord(scrubbed, NAME), 'user-JSON topography is name-free');

  // SYSTEM side: buildFixedAssignmentsBlock echoes the topography verbatim.
  const withScrubbed = buildFixedAssignmentsBlock({ perBehavior: { Climbing: { function: 'escape', topography: scrubbed } }, perSkill: {} });
  assert.ok(!hasWord(withScrubbed, NAME), 'fixed-assignments block is name-free after scrub');
  // Contrast: the RAW topography WOULD carry the name into that same block — proving the scrub is load-bearing.
  const withRaw = buildFixedAssignmentsBlock({ perBehavior: { Climbing: { function: 'escape', topography: rawTopo } }, perSkill: {} });
  assert.ok(hasWord(withRaw, NAME), 'raw topography leaks the name into the block (what the scrub prevents)');
});
