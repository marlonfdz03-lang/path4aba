...
// ── 9. Progress report ready ───────────────────────────────────────────────

export async function sendProgressReportReadyEmail(to: string, clientCode: string, periodLabel: string, role: 'rbt' | 'bcba') {
  const roleLabel = role === 'bcba' ? 'BCBA' : 'RBT'
  const html = `
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:24px">
      <img src="https://path4aba.app/logo.png" alt="Path4ABA" style="height:32px;margin-bottom:24px" />
      <h2 style="color:#0A1628;font-size:20px;margin-bottom:8px">Monthly Progress Report Ready</h2>
      <p style="color:#4B5563;font-size:15px;line-height:1.6">
        The <strong>${periodLabel}</strong> progress report for client <strong>${clientCode}</strong> has been generated and is ready for your review.
      </p>
      <p style="color:#4B5563;font-size:15px;line-height:1.6">
        Sign in to Path4ABA to view the full clinical summary, behavior trends, skill progress, and service justification.
      </p>
      <a href="https://path4aba.app/clients" style="display:inline-block;background:#2563EB;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px;margin:16px 0">
        View Progress Report
      </a>
      <p style="color:#9CA3AF;font-size:13px;margin-top:24px">
        This report was automatically generated by Path4ABA on the first day of the month.<br/>
        Questions? Contact us at <a href="mailto:hello@path4abaapp.com" style="color:#2563EB;">hello@path4abaapp.com</a>
      </p>
    </div>
  `
  return getResend().emails.send({
    from: FROM,
    to,
    subject: `Progress Report Ready — ${clientCode} (${periodLabel})`,
    html,
  })
}
