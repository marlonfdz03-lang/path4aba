...
// ── PROBLEM 2: POSITION, not just presence. The function must fall between the behavior/topography and the
// intervention clause — never after the intervention, at the end, or in the response. A single-behavior ABC
// followed by a skill sentence (skill NAME marks the ABC boundary).
const posNote = (abc) => abc + ' In addition to behavior reduction, skill acquisition targeted Break Request with verbal prompting.';
const POS_BEH = [{ name: 'Throwing Objects', topography: 'throwing objects' }];
const POS_SKILLS = ['Break Request'];
const posMissing = (abc) => findMissingFunctionABCs(posNote(abc), POS_BEH, POS_SKILLS).missing.map((m) => m.name);

test('POSITION: function BEFORE the intervention → PASS', () => {
  const abc = 'When a demand was presented, the client began throwing objects, consistent with the documented escape function; the RBT implemented DRA by offering a break; the client paused.';
  assert.deepEqual(posMissing(abc), []);
});

test('POSITION: function AFTER the intervention → FAIL', () => {
  const abc = 'When a demand was presented, the client began throwing objects; the RBT implemented DRA by offering a break, consistent with the documented escape function; the client paused.';
  assert.deepEqual(posMissing(abc), ['Throwing Objects']);
});

test('POSITION: function at the very END of the ABC → FAIL', () => {
  const abc = 'When a demand was presented, the client began throwing objects; the RBT implemented DRA by offering a break; the client completed the task, consistent with the documented escape function.';
  assert.deepEqual(posMissing(abc), ['Throwing Objects']);
});

test('POSITION: function only in the RESPONSE clause → FAIL', () => {
  const abc = 'When a demand was presented, the client began throwing objects; the RBT implemented DRA; the client responded in a manner consistent with the documented escape function.';
  assert.deepEqual(posMissing(abc), ['Throwing Objects']);
});

test('POSITION: all four function types, each correctly placed before the intervention → PASS', () => {
  const beh = [
    { name: 'Elopement', topography: 'leaving the area' },
    { name: 'Calling Out', topography: 'calling out loudly' },
    { name: 'Grabbing', topography: 'grabbing items' },
    { name: 'Rocking', topography: 'rocking in place' },
  ];
  const note =
    'When a demand was presented, the client began leaving the area, consistent with the documented escape function; the RBT implemented DRA by offering a break; the client returned. ' +
    'When adult attention shifted to a peer, the client began calling out loudly, consistent with the documented attention function; the RBT provided planned ignoring by withholding attention; the client settled. ' +
    'When a preferred item was removed, the client began grabbing items, consistent with the documented tangible function; the RBT implemented NCR by scheduling access; the client waited. ' +
    'During an unstructured moment, the client began rocking in place, consistent with the documented automatic-reinforcement function; the RBT redirected by offering an activity; the client engaged. ' +
    'In addition, skill acquisition targeted Break Request with verbal prompting.';
  assert.deepEqual(findMissingFunctionABCs(note, beh, ['Break Request']).missing, []);
});
