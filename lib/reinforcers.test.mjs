...
test('the real Alexandra shred cases all come out clean', () => {
  ...
  // spot-check some expected clean items survive (from tangibles + social)
  for (const expected of ['strawberries', 'cookies', 'chocolate', 'pizza', 'Great job', 'Nice work', 'Excellent', 'clapping', 'social acknowledgment']) {
    assert.ok(items.some((i) => i.toLowerCase() === expected.toLowerCase()), `expected "${expected}" in ${JSON.stringify(items)}`);
  }
  // the `people` category is EXCLUDED — a caregiver name must never become a reinforcer item.
  assert.ok(!items.some((i) => i.toLowerCase() === 'mother'), '"Mother" (people) must be excluded from reinforcers');
});
