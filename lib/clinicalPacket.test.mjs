...
test('interventions is a REQUIRED domain: located → interventionDomainFound true; absent → flagged missing', () => {
  const withInt = 'Maladaptive Behaviors Summary\nTantrums (active).\nApproved Interventions\nDRA, FCT, NCR, Redirection.\n';
  assert.equal(buildClinicalPacket(withInt).interventionDomainFound, true);
  const withoutInt = 'Maladaptive Behaviors Summary\nTantrums (active). Operational definition: crying.\n';
  const r = buildClinicalPacket(withoutInt);
  assert.equal(r.interventionDomainFound, false);
  assert.ok(r.missing.some((x) => /intervention/i.test(x)));
});

test('FIX 1 anchors: "Behaviors to Increase" + "Behaviors Targeted for Reduction" headings are located', () => {
  // Brandon's BCBA's heading style — matched NONE of the prior anchors, so the roster + reduction list reached
  // the packet only incidentally. Now both required domains are located by their real headings.
  let t = 'Behaviors Targeted for Reduction\nPhysical Aggression (Status: active). Operational definition: hits.\n';
  while (t.length < 4000) t += 'behavior reduction program narrative. ';
  t += '\nBehaviors to Increase\nShare a toy, End structured games, Compliance with daily living, FCT for attention.\n';
  while (t.length < 8000) t += 'replacement program narrative. ';
  t += '\nApproved Interventions\nDRA, DRI, DRO, Redirection.\n';
  const r = buildClinicalPacket(t);
  assert.equal(r.behaviorDomainFound, true, 'reduction heading located');
  assert.equal(r.replacementDomainFound, true, '"Behaviors to Increase" heading located');
  assert.equal(r.interventionDomainFound, true);
});
