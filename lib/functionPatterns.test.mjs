...
// ── Drift record (functionsOutsideAssignedSet): admin-only signal, record-only. Returns the canonical
// functions NAMED in the (already-bounded) text that are NOT in the assigned set.
test('drift: a function named outside the assigned set is recorded', () => {
  assert.deepEqual(functionsOutsideAssignedSet('consistent with the documented attention function', ['escape']), ['attention']);
});

test('drift: a function named inside the assigned set is not recorded', () => {
  assert.deepEqual(functionsOutsideAssignedSet('consistent with the documented escape function', ['escape']), []);
});

test('drift: when all four functions are assigned, nothing can be out of set (blind spot)', () => {
  const text = 'escape function, attention function, tangible function, and automatic reinforcement';
  assert.deepEqual(functionsOutsideAssignedSet(text, ['escape', 'attention', 'tangible', 'automatic']), []);
});

test('drift: a hit that falls AFTER the skill boundary is not recorded (the boundary removes the FP)', () => {
  const note =
    'During the session the client engaged in Elopement, consistent with the documented escape function, ' +
    'before the RBT implemented DRA. Requesting a Break was targeted as the replacement skill; the client ' +
    'engaged in attention-seeking mands appropriately.';
  const behaviors = [{ name: 'Elopement' }];
  const boundary = abcSectionBoundary(note, behaviors, ['Requesting a Break']);
  assert.equal(typeof boundary, 'number');
  // BOUNDED to the ABC section: the post-boundary "attention-seeking" prose is excluded → no drift.
  assert.deepEqual(functionsOutsideAssignedSet(note.slice(0, boundary), ['escape']), []);
  // UNBOUNDED, the same prose would falsely record 'attention' — this is exactly what the boundary prevents.
  assert.deepEqual(functionsOutsideAssignedSet(note, ['escape']), ['attention']);
});
