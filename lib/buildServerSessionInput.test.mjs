...
// ── EDIBLE FIREWALL (clinical safety): edibles never reach the note, on either channel ─────────
// The real bug: a profile leading with edibles put strawberries/cookies/chocolate straight into the note,
// because reinforcersUsed handed them to GPT as locked approved values and the prompt ban never applied.
test('MARLON REQUIRED: edibles are filtered at selection — never reach the note', () => {
  const profile = { ...PROFILE, reinforcers: ['strawberries', 'cookies', 'chocolate', 'fidget toy', 'poker chip', 'tablet'] };
  const input = buildServerSessionInput(SLIM, profile, null);
  const items = input.reinforcersUsed.map((r) => r.item);
  // both downstream channels are clean
  assert.deepEqual(items, ['fidget toy', 'poker chip', 'tablet'], 'edibles gone from reinforcersUsed');
  for (const e of ['strawberries', 'cookies', 'chocolate']) {
    assert.ok(!items.includes(e), `${e} must not appear`);
    assert.ok(!input.clientProfile.reinforcers.tangibles.includes(e), `${e} must not reach tangibles`);
  }
  // the non-edible battery is not over-filtered
  assert.ok(input.clientProfile.reinforcers.tangibles.includes('fidget toy'), 'fidget toy survives');
  assert.ok(input.clientProfile.reinforcers.tangibles.includes('poker chip'), 'poker chip survives');
  // and the type tag is honestly derived (all remaining are non-edible)
  assert.ok(input.reinforcersUsed.every((r) => r.type === 'non-edible'));
});

test('all-edible profile → empty reinforcers → prompt graceful fallback (no item named)', () => {
  const profile = { ...PROFILE, reinforcers: ['cookies', 'candy', 'ice cream', 'juice'] };
  const input = buildServerSessionInput(SLIM, profile, null);
  assert.deepEqual(input.reinforcersUsed, [], 'nothing to name — reinforcersUsed empty');
  assert.equal(input.clientProfile.reinforcers.tangibles, '', 'tangibles empty → prompt falls back to category/contingency');
});
