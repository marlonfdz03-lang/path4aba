...
// (Bug 1, superseded) Read-time reinforcer split. A stored profile persisted before the ingest-time
// " or " split still holds "tablet or phone" as one element, and the builder used to re-split it so the
// note never rendered "(tablet or phone)". The note no longer names profile reinforcers AT ALL — the
// RBT is not asked which items they used, so naming one asserts an unreported clinical fact — which
// subsumes that guarantee: an unresolved alternative cannot reach the note because no profile
// reinforcer can. These now assert the stronger property. splitReinforcerValue is unchanged and still
// tested in reinforcers.test.mjs; it applies again the day the form collects reinforcers.
test('reinforcer read-split (superseded): an unresolved "tablet or phone" cannot reach the note', () => {
  const input = buildServerSessionInput(SLIM, { ...PROFILE, reinforcers: ['tablet or phone', 'Pokémon cards'] });
  assert.deepEqual(input.reinforcersUsed, [], 'no profile reinforcer is named, resolved or not');
  assert.ok(!JSON.stringify(input).includes('tablet or phone'), 'the unresolved alternative reaches nothing');
  assert.ok(!JSON.stringify(input).includes('Pokémon cards'), 'nor does a resolved profile reinforcer');
});
...
// ── REINFORCERS AND ACTIVITIES: not collected, so not named ───────────────────────────────────
// Both used to be filled from the client's profile — four activities and three reinforcers per
// note, stamped preferred/contingent — so the note asserted which activities ran and which items
// were delivered when, none of it reported by anyone.
test('reinforcers and activities are not invented from the profile', () => {
  const input = buildServerSessionInput(SLIM, PROFILE, null);
  assert.deepEqual(input.activitiesUsed, [], 'no activity is named unless the session data provides it');
  assert.deepEqual(input.reinforcersUsed, [], 'no reinforcer is named unless the session data provides it');
  const json = JSON.stringify(input);
  for (const name of ['puzzles', 'coloring', 'fidget tools', 'bean bags']) {
    assert.ok(!json.includes(name), `profile item "${name}" must not reach the note input`);
  }
});

test('who was present is still real input and still reaches the note', () => {
  // The reinforcers block was stripped of profile names, but `people` is the RBT's own selection.
  const input = buildServerSessionInput({ ...SLIM, present: ['Grandmother'] }, PROFILE, null);
  assert.equal(input.clientProfile.reinforcers.people, 'Grandmother');
  assert.equal(input.clientProfile.reinforcers.tangibles, '');
  assert.equal(input.clientProfile.reinforcers.social, '');
});
