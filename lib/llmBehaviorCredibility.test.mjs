...
import { reconcileBehaviors, assessLlmBehaviorCredibility, looksLikeGarbageName, assessReplacementCompleteness, assessInterventionCompleteness, reconcileMastery, stripStoAnnotation } from './llmBehaviorCredibility.ts';
...
test('MASTERY AUTHORITY: an STO-mastered active behavior is demoted out of mastered (protects the RBT form)', () => {
  // Ximena's class: the LLM tagged an active target "mastered" with an STO annotation. It's also in the
  // active/target set → it is ACTIVE → drop from mastered so it never disappears from the note-form list.
  const r = reconcileMastery(
    ['Tantrum', 'Elopement', 'Self Injury Behavior'],
    ['Tantrum (STO#1, STO#2)', 'Elopement (STO#1)', 'Self Injury Behavior (STO#1, STO#2, STO#3)'],
  );
  assert.deepEqual(r.mastered, [], 'all three are active targets → none remain mastered');
  assert.equal(r.demoted.length, 3);
  assert.ok(!r.demoted.some((n) => /STO/.test(n)), 'STO annotations stripped from the demoted names');
});

test('MASTERY AUTHORITY: a genuinely mastered behavior NOT in the active set stays mastered (Brandon Tantrum)', () => {
  // Brandon: "Tantrums (MASTERED)" is a real target-level completion; it is NOT in his active set.
  const r = reconcileMastery(['FCT for attention', 'Following Instructions'], ['Tantrum']);
  assert.deepEqual(r.mastered, ['Tantrum']);
  assert.deepEqual(r.demoted, []);
});

test('stripStoAnnotation removes (STO#…) markers and bare STO#n', () => {
  assert.equal(stripStoAnnotation('Self-Injurious Behavior (STO#1, STO#2, STO#3)'), 'Self-Injurious Behavior');
  assert.equal(stripStoAnnotation('Elopement (STO#1)'), 'Elopement');
  assert.equal(stripStoAnnotation('Tantrum'), 'Tantrum');
});
