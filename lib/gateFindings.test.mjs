...
test('SUPPRESSED coverage (segmentation unsound) produces NO coverage finding — no false defect, no duplicate', () => {
  // degenerate: segmentable true with a raw (untrusted) missing list, marked suppressed
  const degenerate = collectGateFindings({ state: state({ coverage: { segmentable: true, missing: [{ name: 'Elopement' }], suppressed: 'degenerate' } }) });
  assert.deepEqual(degenerate.filter((f) => f.gate === 'coverage'), [], 'no coverage finding off a suppressed (degenerate) reading');
  // unsegmentable: segmentable false, suppressed — must NOT emit the "could not be segmented" info either
  const unseg = collectGateFindings({ state: state({ coverage: { segmentable: false, missing: [], suppressed: 'unsegmentable' } }) });
  assert.deepEqual(unseg.filter((f) => f.gate === 'coverage'), [], 'no coverage finding off a suppressed (unsegmentable) reading');
  // contrast: NON-suppressed missing still files a coverage finding (the check still works when sound)
  const sound = collectGateFindings({ state: state({ coverage: { segmentable: true, missing: [{ name: 'Elopement' }] } }) });
  assert.equal(sound.filter((f) => f.gate === 'coverage').length, 1, 'a real (sound) coverage defect still files');
});
