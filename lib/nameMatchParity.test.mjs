...
const port = new Function(
  `${ported}\nreturn { normName, stripOuterQuotes, matchWords, acronymsOf, sharedAcronym, TIERS, namesMatch, canonicalName, resolveName, resolveOption, buildVariantIndex };`
)();
...
// ── resolveOption: the three-step apply-path resolution (normalized-exact / lone-match / else refuse) ──
test("parity: resolveOption agrees on a battery of (wanted, options)", () => {
  const vectors = [
    ["Off-task behavior", ["Off Task", "Tantrum"]],
    ["Tantrum 2", ["Tantrum", "Tantrum 2"]],
    ["Tantrum 2", ["Tantrum", "Tantrum 22"]],
    ["Self-Injurious Behavior (SIB)", ["Self-Injurious Behavior"]],
    ["Elopement", ["Off Task", "Tantrum"]],
    ["", ["Off Task"]],
  ];
  for (const [w, opts] of vectors) {
    assert.deepEqual(port.resolveOption(w, opts), reference.resolveOption(w, opts), `resolveOption "${w}"`);
  }
});

test("resolveOption: the roster's at-risk names resolve to their terse ABA Matrix option", () => {
  // representative of the 29 (hyphen / trailing 'Behavior' / plural / case) — all previously EXACT-failed
  const cases = [
    ["Off-task behavior", "Off Task"], ["Off-Task Behavior", "Off Task"], ["Off Task Behavior", "Off Task"],
    ["Off-Task Behaviors", "Off Task"], ["Disruptive Behavior", "Disruptive"], ["Impulsive Behavior", "Impulsive"],
    ["Out-of-Seat Behavior", "Out of Seat"], ["Repetitive Behavior", "Repetitive"], ["Perseverative Behavior", "Perseverative"],
    ["Self-Injurious Behaviors", "Self Injurious"], ["Tantrums", "Tantrum"], ["Inappropriate Social Behavior", "Inappropriate Social"],
  ];
  for (const [wanted, opt] of cases) {
    const r = reference.resolveOption(wanted, [opt, "Aggression", "Elopement"]);
    assert.equal(r.status, "matched", `${wanted} -> ${opt} should match, got ${r.status}`);
    assert.equal(r.value, opt);
  }
});

test("resolveOption: the ab802edd SIB pair matches (same behavior, two spellings)", () => {
  assert.equal(reference.namesMatch("Self-Injurious Behavior (SIB)", "Self-Injury Behavior (SIB)", "strict"), true);
});

test("resolveOption: a dropdown offering both Tantrum and Tantrum 2 resolves to Tantrum 2 by exact", () => {
  const r = reference.resolveOption("Tantrum 2", ["Tantrum", "Tantrum 2"]);
  assert.equal(r.status, "matched");
  assert.equal(r.value, "Tantrum 2");
});

test("resolveOption: a genuinely ambiguous case REFUSES (never guesses)", () => {
  const r = reference.resolveOption("Tantrum 2", ["Tantrum", "Tantrum 22"]);
  assert.equal(r.status, "ambiguous");
  assert.deepEqual(r.candidates.sort(), ["Tantrum", "Tantrum 22"]);
  // and a name with no option is 'none' (caller's existing NO_MATCHING_OPTION path)
  assert.equal(reference.resolveOption("Elopement", ["Off Task", "Tantrum"]).status, "none");
});
