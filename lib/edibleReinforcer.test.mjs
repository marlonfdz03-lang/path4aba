...
test('gap fix (live miss): "Vanilla flavor sweets" and plural "Fruits" are now caught', () => {
  for (const s of ['Vanilla flavor sweets', 'sweets', 'sweet', 'Fruits', 'fruit', 'dessert', 'desserts'])
    assert.equal(looksEdible(s), true, `should flag: ${s}`);
  // advisory is now over-inclusive by design (it no longer filters); an innocent non-food that happens to
  // contain none of the new bare words still passes.
  for (const s of ['sensory bin', 'sticker chart', 'building blocks'])
    assert.equal(looksEdible(s), false, `should NOT flag: ${s}`);
});
